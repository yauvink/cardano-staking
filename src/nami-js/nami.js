...
class NamiWalletApi {
  ...
  async getAddress() {
    ...
    const addressHex = Buffer.from((await this.Nami.getUsedAddresses())[0], 'hex');

    const address = this.S.BaseAddress.from_address(this.S.Address.from_bytes(addressHex)).to_address().to_bech32();
  ...
  async getUtxos(utxos) {
    let Utxos = utxos.map((u) => this.S.TransactionUnspentOutput.from_bytes(Buffer.from(u, 'hex')));
    let UTXOS = [];
  ...
  async transaction({
    ...
    let utxos = utxosRaw.map((u) => this.S.TransactionUnspentOutput.from_bytes(Buffer.from(u, 'hex')));
  ...
  async createLockingPolicyScript(address, networkId, expirationTime) {
    ...
    const paymentKeyHash = this.S.BaseAddress.from_address(this.S.Address.from_bytes(Buffer.from(address, 'hex')))
      .payment_cred()
      .to_keyhash();
  ...
  _makeMintedAssets(mintedAssets) {
    ...
    for (const policy in AssetsMap) {
      const ScriptHash = this.S.ScriptHash.from_bytes(Buffer.from(policy, 'hex'));
    ...
    if (value.multiasset()) {
      ...
      for (let j = 0; j < multiAssets.len(); j++) {
        ...
        for (let k = 0; k < assetNames.len(); k++) {
          ...
          const quantity = policyAssets.get(policyAsset);
          const asset =
            Buffer.from(policy.to_bytes()).toString('hex') + '.' + Buffer.from(policyAsset.name()).toString('ascii');
  ...
  async _txBuilderMinting({
    ...
    Object.entries(outputValues).map(([address, value]) => {
      rawOutputs.add(this.S.TransactionOutput.new(this.S.Address.from_bech32(address), value));
    });
    ...
    vkeys.add(this.S.Vkeywitness.from_bytes(Buffer.from(dummyVkeyWitness, 'hex')));
    ...
    Object.entries(outputValues).map(([address, value]) => {
      outputs.add(this.S.TransactionOutput.new(this.S.Address.from_bech32(address), value));
    });
  ...
  async submitTx({ transactionRaw, witnesses, scripts, networkId, metadata }) {
    ...
    const addWitnesses = this.S.TransactionWitnessSet.from_bytes(Buffer.from(witnesses[0], 'hex'));
    ...
    const signedTx = await this.S.Transaction.new(transaction.body(), totalWitnesses, aux);
  ...
  async _blockfrostRequest({ body, endpoint = '', networkId = 0, headers = {}, method = 'GET' }) {
    let networkEndpoint =
      networkId == 0 ? 'https://cardano-testnet.blockfrost.io/api/v0' : 'https://cardano-mainnet.blockfrost.io/api/v0';
